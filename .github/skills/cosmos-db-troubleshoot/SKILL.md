...
| Property | Value |
|----------|-------|
| Account | <set-after-provision> |
| Endpoint | https://<set-after-provision>.documents.azure.com:443/ |
| Database | word-game |
| Resource Group | word-game-dev-rg |
...
```bash
WAF="https://word-game-waf.<set-after-provision>.centralus.azurecontainerapps.io"
TOKEN="<bearer-token>"
...
```bash
az containerapp exec --name word-game-api -g word-game-dev-rg \
  --command "python3 -c \"
import asyncio
...
Navigate to:
```
Azure Portal → <set-after-provision> → Data Explorer → word-game → <container> → Items
```
