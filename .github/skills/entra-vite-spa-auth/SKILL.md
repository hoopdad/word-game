...
1. **Rebuild web**: `az acr build --registry <set-after-provision> --image word-game-web:<tag> --build-arg VITE_MSAL_CLIENT_ID=<set-after-provision> --build-arg VITE_MSAL_AUTHORITY=https://login.microsoftonline.com/<set-after-provision> --build-arg VITE_MSAL_API_CLIENT_ID=<set-after-provision> ../word-game-web`
2. **Deploy web**: `az containerapp update --name word-game-web -g word-game-dev-rg --image <set-after-provision>.azurecr.io/word-game-web:<tag>`
...
az ad app show --id <set-after-provision> \
...
```
WEB_CLIENT_ID=<set-after-provision>
API_CLIENT_ID=<set-after-provision>
TENANT_ID=<set-after-provision>
AUTHORITY=https://login.microsoftonline.com/<set-after-provision>
SCOPE=api://<set-after-provision>/access_as_user
WAF_FQDN=word-game-waf.<set-after-provision>.centralus.azurecontainerapps.io
REDIRECT_URI=https://word-game-waf.<set-after-provision>.centralus.azurecontainerapps.io/welcome
```
