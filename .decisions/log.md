...
2026-06-20 | bugfix: Game-flow batch (5 bugs) fixed via MCP child-agent-runner (phase=full, api+web). Root cause: server WS event/payload shapes diverged from web client. API now emits round_starting, enriched round_started (round_number/guesser_name/category/status), word_shown to clue-givers ONLY (send_to_user, never the guesser — anti-cheat), word in score_updated, category_overview as list, canonical round_ended/game_ended (winners+named scores), display_name in guess_submitted, and /api/game/status returns current_category/current_guesser_name/your_user_id/your_role. Web: Dashboard navigates all players on game_started (auto-join), GameScreen hydrates from GET /api/game/status on mount (idle→/dashboard, active→role-specific), renders role+category badges, handles word_shown, defensive game_ended render + 5s redirect. Contracts websocket-api.yml + game-api.yml updated to canonical shapes.
